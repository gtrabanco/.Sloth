...
dotly::list_bash_files() {
  ...
  find "${SLOTH_PATH:-${DOTLY_PATH:-}}"/{bin,dotfiles_template,scripts,shell} -type f -name "*.sh" -not -path "*template*" -print0 2> /dev/null | xargs -0 -I _ echo _
}
...
dotly::list_dotfiles_bash_files() {
  ...
  find "${DOTFILES_PATH}/"{bin,scripts,shell,restoration_scripts} -type f -name "*.sh" "*/template" -not -path "*template*" -print0 2> /dev/null | xargs -0 -I _ echo _
}
